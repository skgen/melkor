    ...
    >
      ...
      <PopoverPortal>
        <PopoverContent :side-offset="4" side="bottom" align="center" :style="{ zIndex: 'var(--mk-floating-layer-z-index)' }">
          ...
          >
            <span v-if="currentCollection?.categories.length" class="mk-AppInputIcon-menu-counter">
              {{ loadedSize }}/{{ collectionSize }}
            </span>
            <div ref="menuContentRef" class="mk-AppInputIcon-menu-content">
              <template v-if="currentCollection">
                <div
                  v-for="category of currentCollection.categories"
                  :key="category.label"
                  class="mk-AppInputIcon-menu-category"
                >
                  <span class="mk-AppInputIcon-menu-category-title">
                    {{ category.label }}
                  </span>
                  <div class="mk-AppInputIcon-menu-category-grid">
                    <button
                      v-for="icon of category.icons"
                      :key="icon"
                      class="mk-AppInputIcon-menu-category-icon"
                      :data-is-active="`${currentCollection?.name}:${icon}` === props.value || undefined"
                      @click="() => handleChange(`${currentCollection?.name}:${icon}`)"
                    >
                      <AppIcon :icon="`${currentCollection.name}:${icon}`" inline />
                    </button>
                  </div>
                </div>
              </template>
              <div v-else class="mk-AppInputIcon-menu-empty">
                No data
              </div>
            </div>
          </div>
...
import type { IconCollection } from '../../features/icons';
...
import { useIconCollection } from '../../composables/useIconCollection';
import { useInput } from '../../composables/useInput';
import { useTheme } from '../../composables/useTheme';
import { bindInteractionStateProps } from '../../features/interactions';
import { inputIconDefaultProps, type InputIconEmits, type InputIconExpose, type InputIconProps, type InputIconSlots, type InputIconValue } from '../../features/io/input-icon';
import { formatErrors } from '../../features/utils';

export type Props = InputIconProps;
export type Emits = InputIconEmits;
export type Slots = InputIconSlots;
export type Expose = InputIconExpose;
