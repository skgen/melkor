import type { Property, PropertyAST } from '../src/lib/components';
...
export function generateComponentPropsSchema(component: string, debug = false): 0 | 1 | 2 {
  ...
  try {
    ...
    const formattedSchema = JSON
      ...
      .replaceAll('#/definitions/Props', `#/definitions/_${component}Props`);
    ...
    if (e instanceof RootlessError) {
      console.log(gray(`🟨 ${component} has no Props.`));
      return 2;
    }
    else {
      console.error(e);
...
function transformComponentProps(data: unknown): PropertyAST {
  const rawProps = componentRawPropsSchema.parse(data);
  const { Props, ...otherProps } = rawProps.definitions;

  const propertyAST: PropertyAST = {
    props: createProperty(Props, rawProps.definitions) as { type: 'object' } & Property,
  };
  for (const [key, value] of Object.entries(otherProps)) {
    propertyAST[key] = createProperty(value, rawProps.definitions);
  }
  return propertyAST;
}

function createProperty(propertyType: RawProperty, rootPropertyType: Record<string, RawProperty>): Property {
...
// interface PropertyAST {
//   props: { type: 'object' } & Property;
//   [key: string]: Property;
// }
